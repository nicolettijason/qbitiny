...
function FileTree({ files, hash, onPriorityChange, sizeUnit }: FileTreeProps) {
...
export function TorrentFilesDialog({
	...
	const selectedSize =
		files?.filter((f) => f.priority > 0).reduce((acc, f) => acc + f.size, 0) ||
		0;
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
				...
				<div className="flex items-center justify-between text-sm text-muted-foreground px-4 py-2 border-b">
					...
					<span>
						Selected: {selectedCount} ({formatSize(selectedSize, sizeUnit)} /{" "}
						{formatSize(totalSize, sizeUnit)})
					</span>
				</div>
